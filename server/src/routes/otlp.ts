...
import { routeLogRecords } from "../services/event-router";
...
router.post(
  ...
  (async (req: Request, res: Response) => {
    ...
    // Write raw payload to disk (fire-and-forget)
    writeRawPayload("logs", bodyStr);

    try {
      const parsed = JSON.parse(bodyStr);
      await routeLogRecords(parsed);
    } catch (err) {
      appLogger.error({ err }, "Failed to parse/route log records");
    }
